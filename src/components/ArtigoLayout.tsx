...
/**
 * Layout compartilhado dos artigos do blog.
 *
 * Os 5 artigos eram ~87% idênticos entre si (270 das ~315 linhas de cada
 * arquivo). Cada página agora só declara seu conteúdo e chama este layout —
 * publicar um artigo novo deixou de significar copiar 270 linhas.
 */

export type ArtigoRelacionado = {
  title: string;
  category: string;
  image: string;
  href: string;
...
export interface ArtigoLayoutProps {
  /** URL canônica, usada nos links de compartilhamento. */
  url: string;
  title: string;
  subtitle: string;
  category: string;
  ...
  youtubeUrl: string;
  tags: string[];
  relatedPosts: ArtigoRelacionado[];
  /** Link do rodapé "próximo artigo". */
...
const ArtigoLayout = ({
  ...
  const shareLinks = [
    ...
    { label: "Instagram", icon: Heart, href: "https://www.instagram.com/" },
    { label: "Copiar link", icon: Link2, href: url },
  ];
            ...
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div className="min-w-0">
                <div className="mb-8 rounded-2xl bg-pastel-yellow/25 p-6 text-foreground shadow-sm">
                  <p className="font-heading text-lg font-bold text-primary">{pullQuote}</p>
                </div>
                ...
                <div className="mt-8 grid gap-4 border-t border-border pt-6 sm:grid-cols-2">
                  ...
                  <Link to={next.to} className="group flex items-center justify-end gap-3 rounded-2xl bg-card p-4 text-right shadow-sm transition-colors hover:bg-secondary/60">
                    <div>
