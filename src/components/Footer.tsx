...
import { REDES_SOCIAIS, ROTAS_LEGAIS } from "@/lib/site";
...
const socialIcons: Record<string, ({ className }: { className?: string }) => JSX.Element> = {
      ...
      <div className="container py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <div className="flex gap-3">
              {REDES_SOCIAIS.map((rede) => {
                const Icon = socialIcons[rede.label];
                return (
                  <a
                    key={rede.label}
                    href={rede.href}
                    ...
                    className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-foreground/60 hover:text-primary hover:bg-pink transition-colors"
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[
                { label: "Política de Privacidade", path: ROTAS_LEGAIS.privacidade },
                { label: "Termos de Uso", path: ROTAS_LEGAIS.termos },
                { label: "Contato", path: ROTAS_LEGAIS.contato },
              ].map((link) => (
                <li key={link.path}>
                  <Link to={link.path} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {link.label}
